...
Fetches raw pricing data from all cloud provider APIs.
Returns a dict keyed by source name, ready to be passed into normalize_all().

Each function mirrors the logic from fetch_raw_apis_v4.py but is
refactored into clean, reusable async-friendly callables.

REQUIRED ENV VARS (add to .env):
  DATABASE_URL          – PostgreSQL connection string
  INFRACOST_API_KEY     – from: infracost auth login
  GCP_API_KEY           – Google Cloud Billing API key
  COMMODITIES_API_KEY   – (optional) commodities-api.com
  API_NINJAS_KEY        – (optional) api-ninjas.com
"""
...
# ── timeouts ──────────────────────────────────────────────────────────────
_TIMEOUT_SHORT = 15
_TIMEOUT_LONG  = 90   # AWS CSVs can be large
...
MAX_RECORDS = int(os.getenv("PRICING_MAX_RECORDS", "2000"))
...
def _stream_aws_csv(url: str, max_lines: int = 60_000) -> Tuple[List[str], List[Dict]]:
    """
    Stream an AWS Price List CSV and return (headers, rows).
    Uses csv.reader (RFC 4180) to correctly handle embedded commas in
    quoted fields — a common source of silent corruption with naive splits.
    """
    ...
    # AWS CSVs have several disclaimer lines before the real "SKU" header row
...
def fetch_infracost() -> Dict:
    if not INFRACOST_KEY:
        logger.warning("INFRACOST_API_KEY not set — skipping Infracost fetch")
...
def fetch_aws_ec2() -> Dict:
    ...
    try:
        ...
        return {
            ...
            "endpoint":              url,
            "csv_columns":           headers,
            "filter_applied":        "TermType=OnDemand, OS=Linux, Tenancy=Shared, Unit=Hrs",
...
def fetch_aws_rds() -> Dict:
    ...
    try:
        ...
        return {
            ...
            "endpoint":                 url,
            "csv_columns":              headers,
            "filter_applied":           "TermType=OnDemand, Unit=Hrs",
            "total_rows_before_filter": len(all_rows),
            "total_rows_after_filter":  len(rows),
            "records_saved":            len(rows),
            "raw_records":              rows,
...
def fetch_gcp() -> Dict:
    if not GCP_KEY:
        logger.warning("GCP API key not set — skipping GCP fetch")
...
def fetch_all() -> Dict[str, Dict]:
    """
    Run all fetchers and return a dict keyed by source name.
    Errors in individual sources are caught and returned as
    {"status": "error", ...} so one failure doesn't abort the whole sync.
    """
    fetchers = {
